subroutine fluxint2(npg)

use vars

real*8 x(1,2),y(3,2),phi(3),dphi(3),psi(npg),ypsi(2),dypsi(2),xb(2),r,jac,dts,pi,wg(npg),qpsi,qp(3)
...
allocate(scafluxi(ni))       !La matriz intt almacena los valores de la integral para cada x (x es un nodo tomado sobre la frontera)
allocate(Q(ni,1))
...
do j=1,ni
  ...
  do k=1,ne                                           !Este ciclo resuelve la primera matriz para un valor de x fijo; para ello se suman las integrales resultantes en cada elemento, k, sobre el borde
    ...
    do i=1,npg
      ...
      dts=-0.5*xb(1)/(pi*r**2)
      scafluxi(j)=scafluxi(j)+dts*qpsi*jac*wg(i)
     
      !write(*,*)intt(j,conect(k,ii))
    enddo
  enddo

  Q(j,1)=stefluxi(j)+scafluxi(j)!originalmente es con +
  
enddo

open(21,file='Calorint.txt',status='unknown')
write(21,*)'Calor en nodos internos:'
do i=1,10
  do k=0,9
    m=10*k+i
    write(21,*)m,Q(m,1)
